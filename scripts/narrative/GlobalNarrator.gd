...
func _ready() -> void:
    ...
    Logger.log("GlobalNarrator", "ready; city narrators initialized: %s" % str(get_child_count()))
...
func _init_city_narrators() -> void:
    ...
    for i in locs.size():
        ...
        Logger.log("GlobalNarrator", "attached MayorNarrator to %s" % code)
...
func econ_tick() -> void:
    ...
    Logger.log("GlobalNarrator", "econ_tick begin (tick=%s)" % str(Sim.tick_count))
    ...
    if any_changed:
        ...
        if loop != null:
            ...
            Logger.log("GlobalNarrator", "econ_tick: markets updated")
        else:
            Logger.log("GlobalNarrator", "econ_tick: no changes")
