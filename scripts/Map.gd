...
func _draw_routes() -> void:
	...
	for key in DB.routes.keys():
			...
			var a_loc := LocationsDB.get(a_id)
			var b_loc := LocationsDB.get(b_id)
			if a_loc == null or b_loc == null:
					continue
			var pa: Vector2 = Vector2(a_loc.mapPos)
			var pb: Vector2 = Vector2(b_loc.mapPos)
...
func _draw_locations() -> void:
	...
	for loc in locs:
			var loc_id: String = String(loc.id)
			var pos: Vector2 = Vector2(loc.mapPos)
			...
			var name_str: String = tr(loc.displayName)
...
func _draw_caravans() -> void:
	...
	for id in players_dict.keys():
			...
			if p.get("moving", false) and p.has("from") and p.has("to"):
					var from_loc := LocationsDB.get(p["from"])
					var to_loc := LocationsDB.get(p["to"])
					if from_loc != null and to_loc != null:
							var from_pos: Vector2 = Vector2(from_loc.mapPos)
							var to_pos: Vector2 = Vector2(to_loc.mapPos)
					...
					var loc_obj := LocationsDB.get(p["loc"])
					if loc_obj == null:
							continue
					pos = Vector2(loc_obj.mapPos)
...
func _gui_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion:
			...
			var found: String = ""
			for loc in LocationsDB.all():
					var p: Vector2 = Vector2(loc.mapPos)
			...
			for loc in LocationsDB.all():
					...
					if p.distance_to(click_pos) <= 18.0:
							emit_signal("location_clicked", String(loc.id))
							break
