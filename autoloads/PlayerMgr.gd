...
func start_travel(id:int, to_loc:String) -> bool:
	...
	var base_ticks = int(DB.routes[key]["ticks"])
	var speed = max(0.1, calc_speed(id))
	var eta = int(ceil(float(base_ticks) / speed))
	p["moving"] = true
	p["from"] = from_loc
	p["to"] = to_loc
	p["eta_left"] = eta
	p["eta_total"] = eta
	p["progress"] = 0.0
	Commander.emit_signal("log", "[%s] traveling %s -> %s (ETA %d)." % [p["name"], from_loc, to_loc, eta])
	return true
